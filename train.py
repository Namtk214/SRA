    ...
    *,
    ...
    def loss_fn(params):
        if block_diversity_loss:
            ...
            loss_div, _, _, _, loss_div_gate, _ = compute_diversity_loss(
                raw_features, diversity_capture_layers, diversity_pairs,
                orth_weight=diversity_orth_weight, mi_weight=diversity_mi_weight,
                disp_weight=diversity_disp_weight, gate_low=diversity_gate_low,
                gate_high=diversity_gate_high,
            )
        else:
            pred = state.apply_fn(
            ...
            loss_div_gate = jnp.array(0.0, dtype=target.dtype)
        loss_gen = jnp.mean((pred - target) ** 2)
        return loss_gen + loss_div * loss_div_gate
        ...
        for step in range(args.steps_per_epoch):
            ...
            if accum_steps <= 1:
                ...
                for a_idx in range(accum_steps):
                    grads_a, rng = pmapped_grad_step(state, (micro_x[a_idx], micro_y[a_idx]), rng)
